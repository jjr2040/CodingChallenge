...
public enum TRCustomPushTransitionMethod: TransitionAnimationable {
    ...
    public func transitionAnimation() -> TRViewControllerAnimatedTransitioning {
        ...
        case let .blixt(view, frame) :
            return CustomBlixtTransitionAnimation(key: view, toFrame: frame)
        case .default :
...
open class CustomBlixtTransitionAnimation: NSObject, TRViewControllerAnimatedTransitioning, TransitionInteractiveable {
    ...
    open func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        UIView.animate(withDuration: transitionDuration(using: transitionContext) - 0.2, delay: 0.2, options: .curveEaseInOut, animations: {
            ...
            case .pop :
                self.keyViewCopy.isHidden = false
                fromVC?.view.layer.frame.origin.x = rightX
                toVC?.view.layer.frame.origin.x = leftX
                self.keyViewCopy.frame = containView.convert(self.keyView.frame, from: self.keyView.superview)
            default :
            ...
            if finished && !self.cancelPop {
                ...
                self.keyViewCopy.isHidden = true
